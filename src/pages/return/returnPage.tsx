...
//                 <Typography variant="body1" component="div">Book Rent ID {rental._id}</Typography>
...
//             </ul>
//           </CardContent>
//           <Button variant="contained" onClick={handelReturn} color="warning">Return</Button>
//         </Card>
//       )}
...
function ReturnPage() {
  ...
  const handleReturnBooks = async () => {
    if (!bookRental || selectedBooks.length === 0) {
      return;
    }
    console.log(bookRental);

    const rentalID = bookRental[0]._id; // Assuming a single rental record for simplicity

    try {
      await axios.post('/api/bookReturns/bookReturn', {
        rentalID,
      ...
      setError('Failed to return books');
      console.error(err);
    }
  };
      ...
      {selectedBooks.length > 0 && (
        <Card variant="outlined" sx={{ marginTop: 2 }}>
          <CardContent>
            ...
            <Button variant="contained" onClick={handleReturnBooks} color="secondary">Return Selected Books</Button>
          </CardContent>
        </Card>
      )}
